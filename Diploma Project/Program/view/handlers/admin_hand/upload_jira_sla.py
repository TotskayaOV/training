...
from view.states import UploadFileSLA
from view.callback import admin_menu_32
from controller import recording_jira_database


@dp.callback_query_handler(admin_menu_32.filter(level_a32='jira_sla'))
async def upload_jira_time(call: CallbackQuery, admin: bool):
    """
    Переход в state загрузки файла SLA Jira
    """
    current_msg = call.message.message_id
    user_id = call.from_user.id
    if admin:
        await dp.bot.edit_message_text(text=f'Загрузите файл данных SLA Jira',
                                    chat_id=user_id, message_id=current_msg, reply_markup=kb_cancel_fsm)
        await UploadFileSLA.next()
    else:
        dp.bot.edit_message_text(text=f'У вас нет доступа к этой функции',
                                 chat_id=user_id, message_id=current_msg)

@dp.message_handler(state=UploadFileSLA.catch_file, content_types=ContentTypes.ANY)
async def jira_sla_catch(message: Message, state: FSMContext):
    if document := message.document:
        await state.update_data({'catch_file': True})
        await document.download(destination_file=f'./cred/{document.file_name}')
        try:
            os.rename(f'./cred/{document.file_name}', './cred/sla.csv')
        except:
            os.remove('./cred/sla.csv')
            os.rename(f'./cred/{document.file_name}', './cred/sla.csv')
        finally:
            data = await state.get_data()
            try:
                result_upload = recording_jira_database('sla')
                ...
                await state.finish()
                if result_upload:
                    await message.answer(text='Данные SLA Jira загружены', reply_markup=kb_admin_lev32)
                else:
                    await message.answer(text='Ошибка загрузки данных SLA Jira. Проверьте файл:'
